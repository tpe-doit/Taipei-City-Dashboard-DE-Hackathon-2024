...
def seg_sample(
    ...
    """
    string: 要處理的地址
    seg_target: 要正規化的部分，比如'city' or 'dist'
    keyword: 做簡單判斷所使用的關鍵字，比如city就是'縣市'
    str_len: 比如市、區、里都是3個字
    """
    new_address = address
    seg_string = ""
    unexpected_string = ","
...
def seg_only_by_regexp(address: str, seg_target: str, pattern: str):
    """
    address: 要處理的地址
    seg_target: 要正規化的目標名稱，比如'city' or 'dist'
    pattern: 要匹配的字串
    """
    new_address = address
    seg_string = ""
...
def edit_distance(word1: str, word2: str):
    ...
    note:
    目前看來edit_dist是不夠的，不能顯示出兩個相像的字更可能這件事。比如在edit_dist當中州 -> 洲 是1，
